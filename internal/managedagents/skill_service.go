...
func (s *Service) CreateSkill(ctx context.Context, principal Principal, displayTitle *string, files []uploadedSkillFile) (map[string]any, error) {
	fallbackDirectory := ""
	if displayTitle != nil {
	...
	if err := s.repo.CreateSkillWithVersion(ctx, principal.TeamID, skill, version, now); err != nil {
		return nil, err
	}
	return skill, nil
}

func (s *Service) ListSkills(ctx context.Context, principal Principal, limit int, page, source string) ([]map[string]any, *string, bool, error) {
...
func (s *Service) GetSkill(ctx context.Context, principal Principal, skillID string) (map[string]any, error) {
...
func (s *Service) CreateSkillVersion(ctx context.Context, principal Principal, skillID string, files []uploadedSkillFile) (map[string]any, error) {
	...
	versionValue := strconv.FormatInt(now.UnixMicro(), 10)
	version := buildSkillVersionObject(NewID("skillver"), skillID, versionValue, parsed, now)
	if err := s.repo.CreateSkillVersion(ctx, principal.TeamID, skillID, version, now); err != nil {
		return nil, err
	}
	return version, nil
}

func (s *Service) ListSkillVersions(ctx context.Context, principal Principal, skillID string, limit int, page string) ([]map[string]any, *string, bool, error) {
	return s.repo.ListSkillVersions(ctx, principal.TeamID, skillID, limit, page)
}

func (s *Service) GetSkillVersion(ctx context.Context, principal Principal, skillID, version string) (map[string]any, error) {
