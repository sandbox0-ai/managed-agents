...
const skillsAPIBetaHeader = "skills-2025-10-02"
...
func RequireManagedAgentsBeta() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if hasManagedAgentsBeta(header) ||
			(isFilesAPIPath(c.Request.URL.Path) && hasFilesAPIBeta(header)) ||
			(isSkillsAPIPath(c.Request.URL.Path) && hasSkillsAPIBeta(header)) {
			c.Next()
			return
		}
		...
		if isSkillsAPIPath(c.Request.URL.Path) {
			writeError(c, http.StatusBadRequest, "invalid_request_error", "anthropic-beta must include managed-agents-2026-04-01 or skills-2025-10-02")
			c.Abort()
			return
		}
...
func hasSkillsAPIBeta(value string) bool {
	for _, item := range strings.Split(value, ",") {
		if strings.EqualFold(strings.TrimSpace(item), skillsAPIBetaHeader) {
			return true
		}
	}
	return false
...
func isSkillsAPIPath(path string) bool {
	normalized := strings.TrimSpace(path)
	return normalized == "/v1/skills" || strings.HasPrefix(normalized, "/v1/skills/")
}
