...
func (r *Repository) GetRuntime(ctx context.Context, sessionID string) (*RuntimeRecord, error) {
	...
	err := r.db(ctx).QueryRow(ctx, `
		SELECT session_id, vendor, region_id, COALESCE(sandbox_id, ''), COALESCE(wrapper_url, ''), workspace_volume_id,
			callback_token, COALESCE(vendor_session_id, ''), runtime_generation,
			bootstrapped_runtime_generation, COALESCE(bootstrap_state_hash, ''),
	...
	`, strings.TrimSpace(sessionID)).Scan(
		...
		&runtime.ControlToken, &runtime.VendorSessionID, &runtime.RuntimeGeneration,
		&runtime.BootstrappedRuntimeGeneration, &runtime.BootstrapStateHash,
		&runtime.ActiveRunID, &runtime.SandboxDeletedAt, &runtime.CreatedAt, &runtime.UpdatedAt,
	)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
...
func (r *Repository) ListRunningRuntimes(ctx context.Context, limit int) ([]*RuntimeRecord, error) {
	...
	rows, err := r.db(ctx).Query(ctx, `
		SELECT r.session_id, r.vendor, r.region_id, COALESCE(r.sandbox_id, ''), COALESCE(r.wrapper_url, ''), r.workspace_volume_id,
			r.callback_token, COALESCE(r.vendor_session_id, ''), r.runtime_generation,
			r.bootstrapped_runtime_generation, COALESCE(r.bootstrap_state_hash, ''),
...
func (r *Repository) UpsertRuntime(ctx context.Context, runtime *RuntimeRecord) error {
	_, err := r.db(ctx).Exec(ctx, `
		INSERT INTO managed_agent_session_runtimes (
			...
			callback_token, vendor_session_id, runtime_generation,
			bootstrapped_runtime_generation, bootstrap_state_hash,
			active_run_id, sandbox_deleted_at, created_at, updated_at
		)
		...
		ON CONFLICT (session_id) DO UPDATE SET
			...
			bootstrapped_runtime_generation = EXCLUDED.bootstrapped_runtime_generation,
			bootstrap_state_hash = EXCLUDED.bootstrap_state_hash,
	...
	`, runtime.SessionID, runtime.Vendor, runtime.RegionID, nullableString(runtime.SandboxID), runtime.WrapperURL, runtime.WorkspaceVolumeID,
		runtime.ControlToken, nullableString(runtime.VendorSessionID), runtime.RuntimeGeneration,
		runtime.BootstrappedRuntimeGeneration, runtime.BootstrapStateHash,
		runtime.ActiveRunID, runtime.SandboxDeletedAt, runtime.CreatedAt, runtime.UpdatedAt)
	if err != nil {
